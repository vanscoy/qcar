...
class Camera2D():
    def __init__(self, camera_id="0", frame_width=640, frame_height=480, frame_rate=30.0):
        ...
        try:
            self.capture = VideoCapture(self.url, frame_rate, frame_width, frame_height, ImageFormat.ROW_MAJOR_INTERLEAVED_BGR, ImageDataType.UINT8, self.image_data, 0)
            self.capture.start()
        except MediaError as me:
            print(me.get_error_message())
...
class LIDAR():
    ...
    def read(self):
        try:
            self.lidar.read(RangingMeasurementMode.INTERPOLATED, 0.05, np.pi, self.measurements)
