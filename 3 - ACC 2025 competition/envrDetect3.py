...
def detectStopSign(image, showImage=False):
    ...
    for contour in contours:
        ...
        # Check if the contour has 8 vertices (for an octagon)
        if len(approx) == 8:
            # Draw the contour and display text
            cv2.drawContours(image, [approx], 0, (0, 255, 0), 3)
            #cv2.putText(image, "Stop Sign", tuple(approx[0][0]), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
            
            # Return true if an octagon is detected
            return 1
    
    if showImage:
        cv2.imshow('Stop Sign Bouding Box', image)
        cv2.imshow('Stop Sign Red', edges)
        #cv2.waitKey(1)  # Wait for a brief moment
    # Return false if no stop sign is detected
    return 0
...
def detectStopLight(image, showImage = False):
    ...
    area_threshold = 750  # Set a minimum area threshold
