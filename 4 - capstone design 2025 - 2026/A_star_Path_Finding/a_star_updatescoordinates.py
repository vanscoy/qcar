...
# Competition map edge list (weights in arbitrary units)
...
# Scale all edges from arbitrary units to meters
EDGE_SCALE = 14.1667
edges = [(u, v, w * EDGE_SCALE) for u, v, w in edges]
...
def visualize_graph(graph, path=None, out_fn='a_star_map.png'):
    ...
    if nx is None:
        ...
        return
    G = nx.DiGraph()
    # Add edges using the edge list so weights match the updated values
    for u, v, w in edges:
    ...
    # Rotate positions 90 degrees counterclockwise: (x, y) -> (-y, x)
    ...
    edge_labels = {(u, v): f"{w:.1f}m" for u, v, w in edges if G.has_edge(u, v)}
...
def main():
    ...
    # Collect waypoints
    waypoints = []
    print("\nEnter waypoints in order. Type 'done' when finished.")
    ...
    # Get goals
    while True:
        goal = input(f"Enter next goal node (or 'done' to finish): ").strip().upper()
        if goal == 'DONE':
            break
