...
RAW_EDGES = [
    ...
    ("C_2", "Y",   "Y", 2.771, EXIT_ENCODER, None,       None),
    ...
    ("D_1", "G",   "S", 2.258, EXIT_YELLOW_RUNOUT_BOTTOM, None, None),
...
def main():
    ...
    route_nodes, route_modes, route_dists, route_exits, route_left_steers, route_follow_cams = plan_full_route(graph, dropoff)
    dwell_nodes = {dropoff}
    ...
    try:
        ...
        while True:
            ...
            left_thresh = np.zeros((LEFT_FRAME_H, LEFT_FRAME_W), dtype=np.uint8)
            ...
            if state == State.IDLE:
                ...
                if use_left_cam:
                    if left_img is None:
                        ...
                        steering_cmd, forward_cmd, left_overlay, left_thresh, status, white_found = left_follow_step(left_img)
                        calc_ms = (time.time() - t0) * 1000.0
