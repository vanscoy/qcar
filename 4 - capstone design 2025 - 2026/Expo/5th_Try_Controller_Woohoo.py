...
# - Left Camera2D for selected straight segments
...
# Used for modes: S and R unless overridden
# ==========================================================
RIGHT_CAM_ID    = "0"
RIGHT_FRAME_W   = 640
RIGHT_FRAME_H   = 480
...
# ==========================================================
# Left camera / left-straight follower config
# Used only for selected S segments
# ==========================================================
LEFT_CAM_ID    = "2"
LEFT_FRAME_W   = 640
LEFT_FRAME_H   = 480
...
LEFT_TARGET_OFFSET = 50
LEFT_STEERING_GAIN = 0.02
LEFT_STEER_CLIP    = 0.5
LEFT_STEERING_INVERT = True

LEFT_SPEED_MAX = 0.078
LEFT_SPEED_MIN = 0.072
LEFT_SPEED_KP  = 8.16e-05

WIN_LEFT = "Left Camera View"
...
# Map / planner
# Each edge is:
#   (u, v, drive_mode, seg_dist_m, exit_mode, left_steer_override, follow_cam_override)
#
# follow_cam_override:
#   None   -> use normal camera for that mode
#   "left" -> use left camera for this segment
# ==========================================================
NODES = {
    "HUB": (0.0, 0.0, 0.0),
    "A":   (0.0, 0.0, 0.0),
...
RAW_EDGES = [
    ("A",   "C_2", "R", 1.758, EXIT_FIND_YELLOW, None,   None),
    ("A",   "D_2", "L", 2.248, EXIT_FIND_YELLOW, 0.183,  None),

    ("C_2", "Y",   "Y", 2.171, EXIT_ENCODER, None,       None),

    ("C_1", "G",   "L", 2.248, EXIT_YELLOW_RUNOUT_BOTTOM, 0.18, None),
    ("C_1", "D_2", "S", 2.157, EXIT_FIND_YELLOW, None,   None),

    ("D_2", "X",   "Y", 2.376, EXIT_ENCODER, None,       None),

    ("D_1", "G",   "R", 1.958, EXIT_YELLOW_RUNOUT_BOTTOM, None, None),
    ("D_1", "C_2", "S", 2.157, EXIT_FIND_YELLOW, None,   "left"),

    ("G",   "E_2", "R", 1.758, EXIT_FIND_YELLOW, None,   None),
    ("G",   "F_2", "L", 2.248, EXIT_FIND_YELLOW, 0.18,   None),

    ("E_2", "D_1", "Y", 2.714, EXIT_YELLOW_END, None,    None),

    ("E_1", "F_2", "S", 2.757, EXIT_FIND_YELLOW, None,   None),
    ("E_1", "A",   "L", 2.248, EXIT_YELLOW_RUNOUT_BOTTOM, 0.22, None),

    ("F_1", "A",   "R", 1.758, EXIT_YELLOW_RUNOUT_BOTTOM, None, None),
    ("F_1", "E_2", "S", 2.157, EXIT_FIND_YELLOW, None,   "left"),

    ("F_2", "B",   "Y", 1.038, EXIT_ENCODER, None,       None),

    ("B",   "HUB", "Y", 3.534, EXIT_ENCODER, None,       None),
    ("HUB", "C_1", "Y", 0.810, EXIT_YELLOW_END, None,    None),

    ("X",   "E_1", "Y", 1.740, EXIT_YELLOW_END, None,    None),
    ("Y",   "F_1", "Y", 2.063, EXIT_YELLOW_END, None,    None),
]
...
def get_gamepad_button_states(gamepad):
    states = {}
    for label, attr in MISSION_BUTTON_ATTRS.items():
...
def wait_for_node_button(gamepad, prompt_text, disallow=None):
    ...
    prev_states = get_gamepad_button_states(gamepad)
    print(prompt_text)
    print("Use controller buttons: A / B / X / Y")

    while True:
        gamepad.read()
        cur_states = get_gamepad_button_states(gamepad)

        for btn_name, pressed in cur_states.items():
            was_pressed = prev_states.get(btn_name, False)
...
def build_graph(raw_edges):
    nodes = set()
    for u, v, _mode, _segd, _exit_mode, _steer, _follow_cam in raw_edges:
        nodes.add(u)
        nodes.add(v)

    graph = {n: [] for n in nodes}
    for u, v, mode, seg_dist_m, exit_mode, steer_override, follow_cam_override in raw_edges:
        w = 1.0
        graph[u].append((v, w, mode, seg_dist_m, exit_mode, steer_override, follow_cam_override))
    return graph
...
def dijkstra(graph, start, goal):
    ...
    while pq:
        ...
        for v, w, mode, seg_dist_m, exit_mode, steer_override, follow_cam_override in graph.get(u, []):
            ng = g + w
            if ng < best.get(v, float("inf")):
                best[v] = ng
                parent[v] = u
                parent_edge[v] = (u, v, mode, seg_dist_m, exit_mode, steer_override, follow_cam_override, w)
                heappush(pq, (ng, v))
...
def plan_full_route(graph, pickup, dropoff):
    ...
    for i, (s, t) in enumerate(legs):
        ...
        for (_u, _v, mode, seg_dist_m, exit_mode, steer_override, follow_cam_override, _w) in steps:
            full_modes.append(mode)
            full_dists.append(seg_dist_m)
            full_exits.append(exit_mode)
            full_left_steers.append(steer_override)
            full_follow_cams.append(follow_cam_override)

    return full_nodes, full_modes, full_dists, full_exits, full_left_steers, full_follow_cams
...
# ==========================================================
# Left-camera straight follower
# ==========================================================
def left_follow_step(img):
    h, w, _ = img.shape
    display = img.copy()
    ...
    proc = img[crop_y:bottom_crop, crop_x:right_crop]
    gray = cv2.cvtColor(proc, cv2.COLOR_BGR2GRAY)
    _, thresh = cv2.threshold(gray, WHITE_THRESH, 255, cv2.THRESH_BINARY)
    contours, _ = cv2.findContours(thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    steering = 0.0
    speed_cmd = LEFT_SPEED_MIN
    found = False
    status = "NO LEFT LINE DETECTED"

    if not HEADLESS:
        cv2.rectangle(display, (crop_x, crop_y), (right_crop - 1, bottom_crop - 1), (0, 255, 255), 2)
        cv2.line(display, (crop_x, crop_y), (crop_x, bottom_crop - 1), (0, 0, 255), 2)
        cv2.line(display, (right_crop - 1, crop_y), (right_crop - 1, bottom_crop - 1), (0, 255, 0), 2)
    ...
    if contours:
        ...
        if M.get("m00", 0) > 0:
            ...
            target_x = int(w * 0.5) + LEFT_TARGET_OFFSET
            target_y = crop_y + (crop_h // 2)

            dy = int(centroid_full[1]) - int(target_y)
            if abs(dy) > 10:
                steering = float(np.clip(dy * LEFT_STEERING_GAIN, -LEFT_STEER_CLIP, LEFT_STEER_CLIP))
            else:
                steering = 0.0

            steering_cmd = -steering if LEFT_STEERING_INVERT else steering

            prop = abs(target_y - int(centroid_full[1])) + 1
            speed_cmd = float(np.clip(LEFT_SPEED_MAX - (LEFT_SPEED_KP * float(prop)),
                                      LEFT_SPEED_MIN, LEFT_SPEED_MAX))

            status = f"FOUND | dy={dy:+d} steer={steering_cmd:+.3f} speed={speed_cmd:.3f}"

            if not HEADLESS:
                cv2.drawContours(display, [contour_full], -1, (255, 0, 0), 2)
                cv2.circle(display, centroid_full, 10, (255, 0, 0), -1)
                cv2.circle(display, (target_x, target_y), 10, (0, 0, 255), -1)

                cv2.putText(display, f"dy: {dy:+d}",
                            (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 200, 255), 2)
                cv2.putText(display, f"steer: {steering_cmd:+.3f}",
                            (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
                cv2.putText(display, f"speed: {speed_cmd:.3f}",
                            (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (200, 200, 200), 2)

            return steering_cmd, speed_cmd, display, thresh, status, found

    return 0.0, LEFT_SPEED_MIN, display, thresh, status, found
...
def main():
    ...
    leftCam = Camera2D(
        ...
        frame_width=LEFT_FRAME_W,
        frame_height=LEFT_FRAME_H,
        frame_rate=LEFT_FRAME_FPS
    )
    ...
    route_nodes, route_modes, route_dists, route_exits, route_left_steers, route_follow_cams = plan_full_route(graph, pickup, dropoff)
    ...
    print("Follow cams:", route_follow_cams)
    ...
    try:
        ...
        while True:
            ...
            left_img = None
            left_overlay = np.zeros((LEFT_FRAME_H, LEFT_FRAME_W, 3), dtype=np.uint8)
            try:
                leftCam.read()
                if leftCam.image_data is not None and leftCam.image_data.size != 0:
                    left_img = leftCam.image_data.copy()
                    left_overlay = left_img.copy()
            except Exception:
                left_img = None
            ...
            cur_follow_cam = route_follow_cams[step] if step < len(route_follow_cams) else None
            ...
            if state == State.IDLE:
                braking = True
                cv2.putText(overlay, "STATE: IDLE", (10, 30),
                ...
                if step >= len(route_nodes) - 1:
                    ...
                    cur_follow_cam = route_follow_cams[step]
                ...
                use_left_cam = (cur_follow_cam == "left")
                ...
                if use_left_cam:
                    if left_img is None:
                        ...
                        forward_cmd = 0.0
                        white_found = False
                        status = "LEFT CAMERA MISSING"
                        calc_ms = (time.time() - t0) * 1000.0
                        braking = True
                    else:
                        steering_cmd, forward_cmd, left_overlay, _left_thresh, status, white_found = left_follow_step(left_img)
                        calc_ms = (time.time() - t0) * 1000.0

                        if not white_found:
                            forward_cmd = LEFT_SPEED_MIN
                            steering_cmd = 0.0
                else:
                ...
                cv2.putText(overlay, f"STATE: WHITE_FOLLOW | {cur_src}->{cur_dst} | cam={'LEFT' if use_left_cam else 'RIGHT'} | {status}",
                            (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
                cv2.putText(overlay, f"exit={cur_exit_mode} seg: {seg_dist_m:.3f}/{cur_dist_target:.3f} m | total: {total_dist_m:.3f} m",
                            (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
            ...
            if not HEADLESS:
                ...
                cv2.imshow(WIN_LEFT, left_overlay)
                ...
                if k in (27, ord('q'), ord('Q')):
                    ...
                    need_new_mission = True
                    neutral_brake(myCar)
                    print("Stopped. (IDLE) -- waiting for new pickup/dropoff")
        ...
        try:
            ...
            pass
        try:
            safe_terminate_camera2d(leftCam)
        except Exception:
            pass
        try:
