    ...
    try:
        ...
        while True:
            ...
            if state == State.IDLE:
                ...
                # New robust logic:
                # Yellow has priority whenever it is visible.
                # If yellow disappears again, fall back to pink.
                if y_found:
                    overlay = y_overlay
                    mask_show = y_mask
                    status = y_status
                ...
                if cur_exit_mode == EXIT_FIND_YELLOW:
                    # For pink segments, only allow exit once:
                    # 1) enough encoder progress happened, and
                    # 2) yellow is currently found
                    if armed_for_visual_exit(seg_dist_m, cur_dist_target):
                ...
                # Right-camera white follow result
                if right_img is None:
                    w_steer = 0.0
                    w_speed = 0.0
                    white_found = False
                ...
                # Once armed, if yellow is seen at least once, lock into yellow mode
                if armed and y_found:
                    right_to_yellow_seen = True

                # Separate latch:
                # only once yellow is actually seen in the bottom strip do we allow
                # "yellow runout bottom" to be used as an exit condition
                if right_to_yellow_seen and bottom_yellow_present:
                    right_to_yellow_bottom_seen = True

                if right_to_yellow_seen:
                    # Yellow now has control
                    overlay = y_overlay
                    mask_show = y_mask
                    ...
                    # Still using right camera to enter the intersection
                    overlay = img.copy()
                    mask_show = np.zeros((100, 300), dtype=np.uint8)
                ...
                # Only allow exit after we have actually switched onto yellow
                # AND seen yellow in the bottom strip at least once
