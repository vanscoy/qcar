...
# - Front Camera3D depth for stop-sign detection
#
# Mission model:
...
STOP_SIGN_COOLDOWN_SECONDS = 3.0
MAX_STOP_DISTANCE_M = 1.0

RED1_LOWER = np.array([0, 150, 120])
RED1_UPPER = np.array([4, 255, 255])
RED2_LOWER = np.array([170, 150, 120])
RED2_UPPER = np.array([174, 255, 255])
...
RAW_EDGES = [
    ...
    ("A",   "D_2", MODE_L, 2.248, EXIT_FIND_YELLOW, 0.183),

    ("C_2", "Y",   MODE_Y, 2.171, EXIT_ENCODER, None),

    ("C_1", "G",   MODE_L, 2.248, EXIT_YELLOW_RUNOUT_BOTTOM, 0.18),
    ("C_1", "D_2", MODE_P, 2.157, EXIT_FIND_YELLOW, None),

    ("D_2", "X",   MODE_Y, 2.376, EXIT_ENCODER, None),

    ("D_1", "G",   MODE_R, 1.958, EXIT_YELLOW_RUNOUT_BOTTOM, None),
    ("D_1", "C_2", MODE_P, 2.157, EXIT_FIND_YELLOW, None),

    ("G",   "E_2", MODE_R, 1.758, EXIT_FIND_YELLOW, None),
    ("G",   "F_2", MODE_L, 2.248, EXIT_FIND_YELLOW, 0.18),

    ("E_2", "D_1", MODE_Y, 2.714, EXIT_YELLOW_END, None),

    ("E_1", "F_2", MODE_P, 2.757, EXIT_FIND_YELLOW, None),
    ("E_1", "A",   MODE_L, 2.248, EXIT_YELLOW_RUNOUT_BOTTOM, 0.22),

    ("F_1", "A",   MODE_R, 1.758, EXIT_YELLOW_RUNOUT_BOTTOM, None),
    ...
    ("X",   "E_1", MODE_Y, 1.740, EXIT_YELLOW_END, None),
...
def safe_terminate_camera3d(cam):
    if cam is None:
        return
    for name in ("terminate_RGB", "terminate_depth", "stop_RGB", "stop_rgb", "stop"):
        if hasattr(cam, name):
            try:
                getattr(cam, name)()
            except Exception:
...
def yellow_follow_step(img):
    ...
    display = img.copy()
    full_mask_show = np.zeros_like(img)
    full_mask_show[y0:h, 0:w] = cv2.cvtColor(mask, cv2.COLOR_GRAY2BGR)
    ...
    cv2.rectangle(full_mask_show, (0, y0), (w - 1, h - 1), (0, 255, 0), 2)
    ...
    if cnts:
        ...
        if area >= YELLOW_MIN_AREA:
            ...
            if hh >= 12:
                ...
                cv2.drawContours(full_mask_show, [contour_full], -1, (255, 255, 0), 2)
                cv2.circle(full_mask_show, (cx_full, cy_full), 8, (255, 255, 0), -1)
                cv2.circle(full_mask_show, (desired_x, y0 + (h - y0)//2), 8, (0, 0, 255), -1)

    return steering, display, full_mask_show, status, found, area_out
...
def pink_follow_step(img):
    ...
    cv2.rectangle(full_mask_show, (0, y0), (w - 1, h - 1), (255, 0, 255), 2)
...
def detect_stop_sign(frame, depth_img):
    if frame is None or depth_img is None:
        return False, math.nan, np.zeros((FRAME_H, FRAME_W, 3), dtype=np.uint8), "NO DEPTH"
    ...
    for cnt in contours:
        ...
        for i in range(1, num_labels):
            ...
            ratio = comp_h / float(comp_w)
            if comp_area > 50 and 0.5 <= ratio <= 2.5:
                textish = True
                break
        ...
        cv2.putText(display, f"STOP {dist:.2f}m",
                    (x, max(20, y - 10)),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)

        cv2.rectangle(mask_show, (x, y), (x + w, y + h), (0, 255, 0), 2)
        return True, dist, mask_show, f"STOP {dist:.2f}m"

    return False, math.nan, mask_show, "NO STOP"
...
def main():
    ...
    stop_sign_active_until = 0.0
    stop_sign_cooldown_until = 0.0
    stop_mask = np.zeros((FRAME_H, FRAME_W, 3), dtype=np.uint8)
    stop_status = "STOP: idle"
    stop_dist_m = math.nan
    ...
    try:
        ...
        while True:
            ...
            # ----------------------------------------------
            # Stop-sign detection only in sensible states
            # ----------------------------------------------
            stop_seen = False
            stop_dist_m = math.nan
            stop_mask = np.zeros_like(img)
            stop_status = "STOP DISABLED IN THIS STATE"

            stop_detection_allowed = (state == State.YELLOW_FOLLOW)

            if stop_detection_allowed and depth_m is not None:
                stop_seen, stop_dist_m, stop_mask, stop_status = detect_stop_sign(img.copy(), depth_m)
            elif stop_detection_allowed:
                stop_status = "STOP: no depth"

            if stop_detection_allowed:
                if (now >= stop_sign_active_until) and (now >= stop_sign_cooldown_until):
                    if stop_seen:
                        stop_sign_active_until = now + STOP_WAIT_SECONDS
                        stop_sign_cooldown_until = stop_sign_active_until + STOP_SIGN_COOLDOWN_SECONDS
            ...
            # Stop-sign override
            # ----------------------------------------------
            if now < stop_sign_active_until:
                braking = True
                forward_cmd = 0.0
                steering_cmd = 0.0
                remain = stop_sign_active_until - now
                cv2.putText(overlay, f"STOP SIGN HOLD {remain:.1f}s",
                            (10, 180), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
            else:
                cv2.putText(overlay, stop_status,
                            (10, 180), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 100, 255), 2)

            if not math.isnan(stop_dist_m):
                cv2.putText(overlay, f"stop_dist={stop_dist_m:.2f}m cooldown={max(0.0, stop_sign_cooldown_until - now):.1f}s",
                            (10, 210), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 100, 255), 2)
