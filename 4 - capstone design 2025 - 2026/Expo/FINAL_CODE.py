#!/usr/bin/env python3
# Real QCar route FSM for ground testing
#
# Uses:
# - QCar
# - Front Camera3D for yellow-line following
# - Front Camera3D for pink-line following on selected segments
# - Right Camera2D for white-line following (S and R segments)
# - Encoder distance as guide / backup
# - Left turn behavior for L, with per-segment steer override
# - Gamepad for pickup / dropoff selection
# - Stop sign detection only in YELLOW_FOLLOW
#
# Mission model:
#   HUB -> pickup -> dropoff -> HUB
#
# Controller mapping:
#   A -> node A
#   B -> node B
#   X -> node X
#   Y -> node Y
#
# Keys:
#   ESC / Q : quit
#   R       : request new pickup/dropoff from gamepad
#   S       : emergency stop -> IDLE
#   SPACE   : manual advance current segment

from Quanser.product_QCar import QCar
from Quanser.q_essential import Camera3D, Camera2D
from Quanser.q_ui import gamepadViaTarget

import cv2
import time
import math
import enum
import numpy as np
import threading
import os
import sys
from heapq import heappush, heappop

# ==========================================================
# Tunables
# ==========================================================
HEADLESS = False

# Route execution motor commands
SPEED_YELLOW   = 0.078
SPEED_PINK     = 0.072
SPEED_STRAIGHT = 0.070
SPEED_TURN     = 0.070
SPEED_SEARCH   = 0.055

STEER_CLIP = 0.5
STRAIGHT_STEER = 0.0

DWELL_SECONDS = 2.0

# Left turn fallback default only
TURN_LEFT_STEER   = 0.177
TURN_LEFT_TIME_S  = 1.00   # fallback only if distance is unset

# If yellow follower loses line too long, slow down
LOST_TIMEOUT_S = 1.0
LOST_SPEED_CMD = 0.035

# Encoder / odometry
TICKS_PER_REV = 31844.0
WHEEL_DIAM_M  = 0.066
WHEEL_CIRC_M  = math.pi * WHEEL_DIAM_M

# ==========================================================
# Front camera yellow follower config
# ==========================================================
FRAME_W, FRAME_H, FRAME_FPS = 1280, 720, 20.0
KERNEL = np.ones((5, 5), np.uint8)
KERNEL3 = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))
KERNEL5 = cv2.getStructuringElement(cv2.MORPH_RECT, (5, 5))

WINDOW = "QCar Ground FSM"
WIN_MASK = "Front Mask"

YELLOW_BOTTOM_FRAC   = 0.40
YELLOW_BAND_FRAC     = 0.20
YELLOW_MIN_BAND_PTS  = 30
YELLOW_MIN_AREA      = 120
YELLOW_TARGET_OFFSET_RIGHT = 1000
YELLOW_STEERING_GAIN = 0.0012

# ==========================================================
# Front camera pink follower config
# ==========================================================
PINK_ROI_TOP_FRAC    = 0.20
PINK_BAND_FRAC       = 0.20
PINK_MIN_BAND_PTS    = 30
PINK_MIN_AREA        = 60
PINK_TARGET_OFFSET_RIGHT = 1000
PINK_STEERING_GAIN   = 0.0012

# Better pink mask tunables
PINK_HSV_LOWER1 = np.array([140, 70, 70], dtype=np.uint8)
PINK_HSV_UPPER1 = np.array([169, 255, 255], dtype=np.uint8)
PINK_HSV_LOWER2 = np.array([170, 70, 70], dtype=np.uint8)
PINK_HSV_UPPER2 = np.array([179, 255, 255], dtype=np.uint8)

# Obstacle detection
OBSTACLE_ROI_X_START  = 0.30
OBSTACLE_ROI_X_END    = 0.70
OBSTACLE_ROI_Y_START  = 0.15
OBSTACLE_ROI_Y_END    = 0.70

OBSTACLE_DEPTH_MIN_M  = 0.05
OBSTACLE_DEPTH_MAX_M  = 0.60
OBSTACLE_DEPTH_THRESHOLD_M = 0.60

OBSTACLE_PIXEL_FRAC   = 0.24
OBSTACLE_MIN_VALID_PX = 80

OBSTACLE_RGB_FALLBACK      = False
OBSTACLE_RGB_MIN_BLOB_AREA = 3000
OBSTACLE_RGB_MAX_BRIGHT    = 80

OBSTACLE_CONSEC_N = 4

# ==========================================================
# Grabber watchdogs
# ==========================================================
MAX_FRAME_AGE_S   = 0.25
STALE_MAX_FRAMES  = 8
MAX_LOOP_TIME_S   = 0.20
RESET_COOLDOWN_S  = 1.0
MAX_ESCALATION    = 3

# ==========================================================
# Right camera / white follower config
# Used for modes: S and R
# ==========================================================
RIGHT_CAM_ID    = "0"
RIGHT_FRAME_W   = 640
RIGHT_FRAME_H   = 480
RIGHT_FRAME_FPS = 30.0

WHITE_TARGET_OFFSET = 50
WHITE_STEERING_GAIN = 0.008
WHITE_STEER_CLIP    = 0.5

WHITE_SPEED_MAX = 0.078
WHITE_SPEED_MIN = 0.072
WHITE_SPEED_KP  = 2.96296e-05

WHITE_THRESH = 150
WIN_RIGHT = "Right Camera View"

RIGHT_TO_YELLOW_ARM_FRAC = 0.50   # for RY, when to start looking for yellow

# ==========================================================
# Arrival / exit logic tuning
# ==========================================================
EXIT_ARM_FRACTION = 0.80
EXIT_HARD_MAX_FRACTION = 1.20

FIND_YELLOW_CONFIRM_FRAMES = 3
FIND_YELLOW_MIN_AREA = 120

YELLOW_END_CONFIRM_FRAMES = 3

RUNOUT_BOTTOM_FRAC = 0.12
RUNOUT_MIN_AREA = 120
RUNOUT_CONFIRM_FRAMES = 3

# ==========================================================
# Stop sign config (only used in YELLOW_FOLLOW)
# ==========================================================
STOP_ENABLE = True
STOP_DURATION_S = 2.0
STOP_COOLDOWN_S = 4.0

RED1_LOWER = np.array([0, 150, 120])
RED1_UPPER = np.array([6, 255, 255])
RED2_LOWER = np.array([170, 150, 120])
RED2_UPPER = np.array([179, 255, 255])

MIN_STOP_AREA = 500
MAX_STOP_AREA = 50000
MIN_STOP_ASPECT_RATIO = 0.7
MIN_STOP_RED_RATIO = 0.6
MAX_STOP_DISTANCE_M = 1.50

# ==========================================================
# Gamepad mission selection
# ==========================================================
GAMEPAD_INDEX = 1

BUTTON_TO_NODE = {
    "A": "A",
    "B": "B",
    "X": "X",
    "Y": "Y",
}

MISSION_BUTTON_ATTRS = {
    "A": "A",
    "B": "B",
    "X": "X",
    "Y": "Y",
}

# ==========================================================
# Exit rule names
# ==========================================================
EXIT_ENCODER = "encoder"
EXIT_YELLOW_END = "yellow_end"
EXIT_FIND_YELLOW = "find_yellow"
EXIT_YELLOW_RUNOUT_BOTTOM = "yellow_runout_bottom"

# ==========================================================
# Drive mode names
# ==========================================================
MODE_Y = "Y"    # front yellow follow
MODE_P = "P"    # front pink then yellow
MODE_S = "S"    # right white follow
MODE_R = "R"    # right white follow
MODE_RY = "RY"  # right white, then switch to yellow after arm distance
MODE_L = "L"    # left turn behavior

# ==========================================================
# Map / planner
# Each edge is:
#   (u, v, drive_mode, seg_dist_m, exit_mode, left_steer_override)
# ==========================================================
NODES = {
    "HUB": (0.0, 0.0, 0.0),
    "A":   (0.0, 0.0, 0.0),
    "B":   (0.0, 0.0, 0.0),
    "C_1": (0.0, 0.0, 0.0),
    "C_2": (0.0, 0.0, 0.0),
    "D_1": (0.0, 0.0, 0.0),
    "D_2": (0.0, 0.0, 0.0),
    "E_1": (0.0, 0.0, 0.0),
    "E_2": (0.0, 0.0, 0.0),
    "F_1": (0.0, 0.0, 0.0),
    "F_2": (0.0, 0.0, 0.0),
    "G":   (0.0, 0.0, 0.0),
    "X":   (0.0, 0.0, 0.0),
    "Y":   (0.0, 0.0, 0.0),
}

RAW_EDGES = [
    ("A",   "C_2", MODE_R, 1.758, EXIT_FIND_YELLOW, None),
    ("A",   "D_2", MODE_L, 2.048, EXIT_FIND_YELLOW, 0.182),

    ("C_2", "Y",   MODE_Y, 2.371, EXIT_ENCODER, None),

    ("C_1", "G",   MODE_L, 2.248, EXIT_ENCODER, 0.175),
    ("C_1", "D_2", MODE_P, 2.157, EXIT_FIND_YELLOW, None),

    ("D_2", "X",   MODE_Y, 2.376, EXIT_ENCODER, None),

    ("D_1", "G",   MODE_RY, 2.458, EXIT_YELLOW_RUNOUT_BOTTOM, None),
    ("D_1", "C_2", MODE_P, 2.357, EXIT_FIND_YELLOW, None),

    ("G",   "E_2", MODE_R, 1.758, EXIT_FIND_YELLOW, None),
    ("G",   "F_2", MODE_L, 2.248, EXIT_FIND_YELLOW, 0.177),

    ("E_2", "D_1", MODE_Y, 2.714, EXIT_YELLOW_END, None),

    ("E_1", "F_2", MODE_P, 2.757, EXIT_FIND_YELLOW, None),
    ("E_1", "A",   MODE_L, 2.0, EXIT_ENCODER, 0.19),

    ("F_1", "A",   MODE_RY, 1.758, EXIT_YELLOW_RUNOUT_BOTTOM, None),
    ("F_1", "E_2", MODE_P, 2.157, EXIT_FIND_YELLOW, None),

    ("F_2", "B",   MODE_Y, 1.038, EXIT_ENCODER, None),

    ("B",   "HUB", MODE_Y, 3.534, EXIT_ENCODER, None),
    ("HUB", "C_1", MODE_Y, 0.810, EXIT_YELLOW_END, None),

    ("X",   "E_1", MODE_Y, 1.590, EXIT_YELLOW_END, None),
    ("Y",   "F_1", MODE_Y, 2.063, EXIT_YELLOW_END, None),
]

# ==========================================================
# FSM
# ==========================================================
class State(enum.Enum):
    IDLE = 0
    ROUTE = 1
    YELLOW_FOLLOW = 2
    PINK_THEN_YELLOW = 3
    WHITE_FOLLOW = 4
    RIGHT_THEN_YELLOW = 5
    TURN_LEFT = 6
    TURN_RIGHT = 7
    STOP_WAIT = 8

# ==========================================================
# Camera helpers
# ==========================================================
def safe_terminate_camera3d(cam):
    if cam is None:
        return
    for name in ("terminate_RGB", "stop_RGB", "stop_rgb", "stop"):
        if hasattr(cam, name):
            try:
                getattr(cam, name)()
            except Exception:
                pass
    try:
        cam.terminate()
    except AttributeError as e:
        if "video3d" not in str(e):
            raise
    except Exception:
        pass

def safe_terminate_camera2d(cam):
    if cam is None:
        return
    for name in ("terminate", "stop"):
        if hasattr(cam, name):
            try:
                getattr(cam, name)()
            except Exception:
                pass

class FrameGrabber:
    def __init__(self):
        self.cam = None
        self.lock = threading.Lock()
        self.frame = None
        self.depth_m = None
        self.running = False
        self.last_ck = None
        self.stale_count = 0
        self.last_good_t = time.time()
        self.last_reset_attempt = 0.0
        self.thread = None

    def _open_cam(self):
        self.cam = Camera3D(
            mode='RGB&DEPTH',
            frame_width_RGB=FRAME_W,
            frame_height_RGB=FRAME_H,
            frame_rate_RGB=FRAME_FPS,
            device_id='0'
        )

    def start(self):
        self._open_cam()
        self.running = True
        self.thread = threading.Thread(target=self._loop, daemon=True)
        self.thread.start()

    def stop(self):
        self.running = False
        try:
            if self.thread:
                self.thread.join(timeout=0.5)
        except Exception:
            pass
        try:
            safe_terminate_camera3d(self.cam)
        except Exception:
            pass
        self.cam = None

    def _checksum(self, img):
        thumb = cv2.resize(img, (64, 36), interpolation=cv2.INTER_AREA)
        return hash(thumb.tobytes())

    def _soft_reset_ok(self):
        return (time.time() - self.last_reset_attempt) >= RESET_COOLDOWN_S

    def _soft_reset(self):
        if not self._soft_reset_ok():
            return False
        self.last_reset_attempt = time.time()
        try:
            safe_terminate_camera3d(self.cam)
        except Exception:
            pass
        try:
            self._open_cam()
            self.last_ck = None
            self.stale_count = 0
            self.last_good_t = time.time()
            return True
        except Exception:
            return False

    def _loop(self):
        while self.running:
            try:
                self.cam.read_RGB()
                self.cam.read_depth(dataMode='m')

                img = self.cam.image_buffer_RGB
                depth = self.cam.image_buffer_depth_m

                if img is not None and img.size != 0:
                    ck = self._checksum(img)
                    if self.last_ck is not None and ck == self.last_ck:
                        self.stale_count += 1
                    else:
                        self.stale_count = 0
                        self.last_good_t = time.time()
                    self.last_ck = ck

                    with self.lock:
                        self.frame = img.copy()
                        self.depth_m = None if depth is None else depth.copy()
            except Exception:
                pass
            time.sleep(0.001)

    def get_frame(self):
        too_old = (time.time() - self.last_good_t) > MAX_FRAME_AGE_S
        too_stale = self.stale_count >= STALE_MAX_FRAMES
        if too_old or too_stale:
            self._soft_reset()
        with self.lock:
            return None if self.frame is None else self.frame.copy()

    def get_depth(self):
        with self.lock:
            return None if self.depth_m is None else self.depth_m.copy()

# ==========================================================
# Gamepad helpers
# ==========================================================
def init_gamepad(index=GAMEPAD_INDEX):
    try:
        pad = gamepadViaTarget(index)
        pad.read()
        print(f"Gamepad connected on controller index {index}")
        return pad
    except Exception as exc:
        print(f"Gamepad unavailable on controller index {index}: {exc}")
        return None

def wait_for_node_button(gamepad, prompt_text, disallow=None):
    if disallow is None:
        disallow = set()

    if gamepad is None:
        raise RuntimeError("Gamepad not available")

    print(prompt_text)
    print("Use controller buttons: A / B / X / Y")

    while True:
        gamepad.read()

        for btn_name, attr in MISSION_BUTTON_ATTRS.items():
            try:
                pressed = bool(getattr(gamepad, attr, 0))
            except Exception:
                pressed = False

            if pressed:
                chosen_node = BUTTON_TO_NODE[btn_name]
                print(f"Detected button {btn_name} -> node {chosen_node}")

                if chosen_node in disallow:
                    print(f"{chosen_node} is not allowed right now. Choose a different button.")
                    time.sleep(0.35)
                    break
                else:
                    while True:
                        gamepad.read()
                        still_pressed = bool(getattr(gamepad, attr, 0))
                        if not still_pressed:
                            break
                        time.sleep(0.01)

                    time.sleep(0.10)
                    return chosen_node

        time.sleep(0.01)

def ask_pickup_dropoff_gamepad(gamepad):
    pickup = wait_for_node_button(
        gamepad,
        "\nSelect PICKUP location",
        disallow={"HUB"}
    )

    dropoff = wait_for_node_button(
        gamepad,
        "\nSelect DROPOFF location",
        disallow={pickup, "HUB"}
    )

    print(f"Mission selected: HUB -> {pickup} -> {dropoff} -> HUB")
    return pickup, dropoff

# ==========================================================
# QCar helpers
# ==========================================================
def neutral_brake(car):
    try:
        car.read_write_std(
            np.array([0.0, 0.0], dtype=np.float64),
            np.array([1, 0, 0, 0, 1, 0, 0, 0], dtype=np.float64)
        )
    except Exception:
        pass

def drive(car, speed_cmd, steering_cmd, brake=False):
    leds = np.array(
        [1, 0, 0, 0, 1, 0, 1 if not brake else 0, 1 if not brake else 0],
        dtype=np.float64
    )
    if brake:
        leds = np.array([1, 0, 0, 0, 1, 0, 0, 0], dtype=np.float64)

    try:
        car.read_write_std(
            np.array([float(speed_cmd), float(steering_cmd)], dtype=np.float64),
            leds
        )
    except Exception:
        pass

def reopen_qcar(car):
    try:
        car.terminate()
    except Exception:
        pass
    try:
        return QCar()
    except Exception:
        return None

def read_ticks(qcar) -> float:
    return float(qcar.read_encoder())

# ==========================================================
# Encoder odometry
# ==========================================================
class SegmentOdom:
    def __init__(self):
        self.prev_ticks = None
        self.segment_dist_m = 0.0
        self.total_dist_m = 0.0

    def reset_all(self, qcar):
        self.prev_ticks = read_ticks(qcar)
        self.segment_dist_m = 0.0
        self.total_dist_m = 0.0

    def reset_segment(self, qcar):
        self.prev_ticks = read_ticks(qcar)
        self.segment_dist_m = 0.0

    def update(self, qcar):
        ticks_now = read_ticks(qcar)

        if self.prev_ticks is None:
            self.prev_ticks = ticks_now
            return 0.0, self.segment_dist_m, self.total_dist_m

        d_ticks = ticks_now - self.prev_ticks
        self.prev_ticks = ticks_now

        d_m = (d_ticks / TICKS_PER_REV) * WHEEL_CIRC_M

        self.segment_dist_m += abs(d_m)
        self.total_dist_m += abs(d_m)

        return d_m, self.segment_dist_m, self.total_dist_m

# ==========================================================
# Planner
# ==========================================================
def build_graph(raw_edges):
    nodes = set()
    for u, v, _mode, _segd, _exit_mode, _steer in raw_edges:
        nodes.add(u)
        nodes.add(v)

    graph = {n: [] for n in nodes}
    for u, v, mode, seg_dist_m, exit_mode, steer_override in raw_edges:
        w = 1.0
        graph[u].append((v, w, mode, seg_dist_m, exit_mode, steer_override))
    return graph

def dijkstra(graph, start, goal):
    pq = [(0.0, start)]
    best = {start: 0.0}
    parent = {start: None}
    parent_edge = {start: None}

    while pq:
        g, u = heappop(pq)
        if g != best.get(u, float("inf")):
            continue
        if u == goal:
            break

        for v, w, mode, seg_dist_m, exit_mode, steer_override in graph.get(u, []):
            ng = g + w
            if ng < best.get(v, float("inf")):
                best[v] = ng
                parent[v] = u
                parent_edge[v] = (u, v, mode, seg_dist_m, exit_mode, steer_override, w)
                heappush(pq, (ng, v))

    if goal not in best:
        return None, float("inf"), None

    path, steps = [], []
    cur = goal
    while cur is not None:
        path.append(cur)
        e = parent_edge[cur]
        if e is not None:
            steps.append(e)
        cur = parent[cur]

    path.reverse()
    steps.reverse()
    return path, best[goal], steps

def plan_full_route(graph, pickup, dropoff):
    legs = [("HUB", pickup), (pickup, dropoff), (dropoff, "HUB")]

    full_nodes = []
    full_modes = []
    full_dists = []
    full_exits = []
    full_left_steers = []

    for i, (s, t) in enumerate(legs):
        path, _cost, steps = dijkstra(graph, s, t)
        if path is None:
            raise RuntimeError(f"No path from {s} to {t}")

        if i == 0:
            full_nodes += path
        else:
            full_nodes += path[1:]

        for (_u, _v, mode, seg_dist_m, exit_mode, steer_override, _w) in steps:
            full_modes.append(mode)
            full_dists.append(seg_dist_m)
            full_exits.append(exit_mode)
            full_left_steers.append(steer_override)

    return full_nodes, full_modes, full_dists, full_exits, full_left_steers

# ==========================================================
# Shared perception helpers
# ==========================================================
def contour_centroid_in_band(largest, roi_shape, band_frac, min_band_pts):
    pts = largest.reshape(-1, 2)
    roi_h = roi_shape[0]
    band_y_start = int(roi_h * (1.0 - band_frac))
    band_pts = pts[pts[:, 1] >= band_y_start]

    if band_pts.shape[0] >= min_band_pts:
        cx = int(band_pts[:, 0].mean())
        cy = int(band_pts[:, 1].mean())
    else:
        N = min(50, pts.shape[0])
        idx = np.argsort(pts[:, 1])[-N:]
        sel = pts[idx]
        cx = int(sel[:, 0].mean())
        cy = int(sel[:, 1].mean())

    return cx, cy

def make_yellow_mask(roi):
    hsv = cv2.cvtColor(roi, cv2.COLOR_BGR2HSV)

    mask = cv2.inRange(hsv, (10, 90, 90), (65, 255, 255))

    white_glare = cv2.inRange(hsv, (0, 0, 210), (180, 70, 255))
    mask = cv2.bitwise_and(mask, cv2.bitwise_not(white_glare))

    lab = cv2.cvtColor(roi, cv2.COLOR_BGR2LAB)
    b = lab[:, :, 2]
    _, b_bin = cv2.threshold(b, 155, 255, cv2.THRESH_BINARY)
    mask = cv2.bitwise_and(mask, b_bin)

    mask = cv2.morphologyEx(mask, cv2.MORPH_OPEN, KERNEL, iterations=1)
    mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, KERNEL, iterations=2)
    return mask

def make_pink_mask(roi_bgr):
    hsv = cv2.cvtColor(roi_bgr, cv2.COLOR_BGR2HSV)

    m1 = cv2.inRange(hsv, PINK_HSV_LOWER1, PINK_HSV_UPPER1)
    m2 = cv2.inRange(hsv, PINK_HSV_LOWER2, PINK_HSV_UPPER2)
    mask = cv2.bitwise_or(m1, m2)

    white_glare = cv2.inRange(hsv, (0, 0, 220), (180, 60, 255))
    mask = cv2.bitwise_and(mask, cv2.bitwise_not(white_glare))

    mask = cv2.morphologyEx(mask, cv2.MORPH_OPEN, KERNEL5, iterations=1)
    mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, KERNEL5, iterations=1)
    return mask

# ==========================================================
# Stop sign helpers
# ==========================================================
def read_distance_scalar(depth_img, cx, cy):
    try:
        if depth_img is None:
            return math.nan
        h, w = depth_img.shape[:2]
        if cx < 0 or cy < 0 or cx >= w or cy >= h:
            return math.nan

        val = depth_img[cy, cx]

        if isinstance(val, np.ndarray):
            if val.size == 0:
                return math.nan
            val = val.flat[0]

        dist = float(val)
        if not math.isfinite(dist) or dist <= 0.0:
            return math.nan
        return dist
    except Exception:
        return math.nan

def detect_stop_sign(frame, depth_img):
    hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)

    mask1 = cv2.inRange(hsv, RED1_LOWER, RED1_UPPER)
    mask2 = cv2.inRange(hsv, RED2_LOWER, RED2_UPPER)
    mask = cv2.bitwise_or(mask1, mask2)

    mask = cv2.GaussianBlur(mask, (5, 5), 0)
    kernel = np.ones((3, 3), np.uint8)
    mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, kernel)
    mask = cv2.morphologyEx(mask, cv2.MORPH_OPEN, kernel)

    contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    best_box = None
    best_dist = math.nan

    for cnt in contours:
        area = cv2.contourArea(cnt)
        if area < MIN_STOP_AREA or area > MAX_STOP_AREA:
            continue

        x, y, w, h = cv2.boundingRect(cnt)
        ar = float(w) / float(h) if h != 0 else 0.0
        if ar < MIN_STOP_ASPECT_RATIO or ar > 1.0 / MIN_STOP_ASPECT_RATIO:
            continue

        roi_mask = mask[y:y + h, x:x + w]
        if roi_mask.size == 0:
            continue

        red_ratio = cv2.countNonZero(roi_mask) / float(w * h)
        if red_ratio < MIN_STOP_RED_RATIO:
            continue

        cx, cy = x + w // 2, y + h // 2
        dist = read_distance_scalar(depth_img, cx, cy)

        if math.isnan(dist) or dist > MAX_STOP_DISTANCE_M:
            continue

        best_box = (x, y, w, h)
        best_dist = dist
        break

    annotated = frame.copy()
    found = best_box is not None

    if found:
        x, y, w, h = best_box
        cv2.rectangle(annotated, (x, y), (x + w, y + h), (0, 255, 0), 3)
        cv2.putText(annotated, f"STOP {best_dist:.2f}m",
                    (x, max(20, y - 10)),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)

    return found, annotated, mask, best_dist

# ==========================================================
# Obstacle helpers
# ==========================================================
def _get_obstacle_roi_coords(frame_h, frame_w):
    return (
        int(frame_w * OBSTACLE_ROI_X_START),
        int(frame_h * OBSTACLE_ROI_Y_START),
        int(frame_w * OBSTACLE_ROI_X_END),
        int(frame_h * OBSTACLE_ROI_Y_END),
    )


def _depth_metrics(depth_m, x0, y0, x1, y1):
    roi = depth_m[y0:y1, x0:x1].astype(np.float32)
    if roi.ndim == 3:
        roi = roi.squeeze(axis=-1)

    mask = np.isfinite(roi) & (roi > OBSTACLE_DEPTH_MIN_M) & (roi < OBSTACLE_DEPTH_MAX_M)
    valid = roi[mask]
    total = roi.size or 1

    p10 = float(np.percentile(valid, 10)) if valid.size >= OBSTACLE_MIN_VALID_PX else math.nan
    frac = valid.size / total
    return valid, p10, frac


def _rgb_contour_obstacle(rgb, x0, y0, x1, y1):
    roi = rgb[y0:y1, x0:x1]
    gray = cv2.cvtColor(roi, cv2.COLOR_BGR2GRAY)
    _, th = cv2.threshold(gray, OBSTACLE_RGB_MAX_BRIGHT, 255, cv2.THRESH_BINARY_INV)
    th = cv2.morphologyEx(th, cv2.MORPH_OPEN, KERNEL5, iterations=1)
    th = cv2.morphologyEx(th, cv2.MORPH_CLOSE, KERNEL5, iterations=2)
    cnts, _ = cv2.findContours(th, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    return any(cv2.contourArea(c) >= OBSTACLE_RGB_MIN_BLOB_AREA for c in cnts)


def detect_obstacle(rgb, depth_m, frame_h, frame_w):
    if depth_m is None:
        if rgb is not None and OBSTACLE_RGB_FALLBACK:
            x0, y0, x1, y1 = _get_obstacle_roi_coords(frame_h, frame_w)
            return _rgb_contour_obstacle(rgb, x0, y0, x1, y1)
        return False

    x0, y0, x1, y1 = _get_obstacle_roi_coords(frame_h, frame_w)
    valid, p10, frac = _depth_metrics(depth_m, x0, y0, x1, y1)

    if valid.size < OBSTACLE_MIN_VALID_PX:
        if rgb is not None and OBSTACLE_RGB_FALLBACK:
            return _rgb_contour_obstacle(rgb, x0, y0, x1, y1)
        return False

    if math.isfinite(p10) and p10 < OBSTACLE_DEPTH_THRESHOLD_M:
        return True
    if frac >= OBSTACLE_PIXEL_FRAC:
        return True
    return False


def obstacle_p10(depth_m, frame_h, frame_w):
    if depth_m is None:
        return math.nan
    x0, y0, x1, y1 = _get_obstacle_roi_coords(frame_h, frame_w)
    _, p10, _ = _depth_metrics(depth_m, x0, y0, x1, y1)
    return p10


def draw_obstacle_roi(disp, frame_h, frame_w, detected, p10_dist=None):
    x0, y0, x1, y1 = _get_obstacle_roi_coords(frame_h, frame_w)
    colour = (0, 0, 255) if detected else (200, 200, 0)
    cv2.rectangle(disp, (x0, y0), (x1, y1), colour, 2)
    if detected:
        label = "OBSTACLE!"
        if p10_dist is not None and math.isfinite(p10_dist):
            label += f" {p10_dist:.2f}m"
        cv2.putText(disp, label, (x0, max(20, y0 - 8)),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 255), 2)

# ==========================================================
# Yellow detection
# ==========================================================
def yellow_follow_step(img):
    h, w, _ = img.shape
    y0 = int(h * (1.0 - YELLOW_BOTTOM_FRAC))
    roi = img[y0:h, 0:w]

    mask = make_yellow_mask(roi)
    cnts, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    display = img.copy()
    found = False
    steering = 0.0
    status = "NO YELLOW"
    area_out = 0.0

    if cnts:
        largest = max(cnts, key=cv2.contourArea)
        area = cv2.contourArea(largest)
        area_out = float(area)

        if area >= YELLOW_MIN_AREA:
            x, y, ww, hh = cv2.boundingRect(largest)
            if hh >= 12:
                cx, cy = contour_centroid_in_band(
                    largest, roi.shape[:2], YELLOW_BAND_FRAC, YELLOW_MIN_BAND_PTS
                )

                cx_full = cx
                cy_full = y0 + cy
                contour_full = largest + np.array([0, y0])

                desired_x = w - YELLOW_TARGET_OFFSET_RIGHT
                error = desired_x - cx_full
                steering = float(np.clip(error * YELLOW_STEERING_GAIN, -STEER_CLIP, STEER_CLIP))

                found = True
                status = f"YELLOW FOUND | err={error:+.1f} steer={steering:+.3f} area={area:.0f}"

                if not HEADLESS:
                    cv2.rectangle(display, (0, y0), (w - 1, h - 1), (0, 255, 0), 2)
                    cv2.drawContours(display, [contour_full], -1, (255, 0, 0), 2)
                    cv2.circle(display, (cx_full, cy_full), 8, (255, 0, 0), -1)
                    cv2.circle(display, (desired_x, y0 + (h - y0)//2), 8, (0, 0, 255), -1)

    return steering, display, mask, status, found, area_out

# ==========================================================
# Pink detection
# ==========================================================
def pink_follow_step(img):
    h, w, _ = img.shape
    y0 = int(h * PINK_ROI_TOP_FRAC)
    roi = img[y0:h, 0:w]

    mask = make_pink_mask(roi)
    cnts, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    display = img.copy()
    found = False
    steering = 0.0
    status = "NO PINK"
    area_out = 0.0

    if cnts:
        largest = max(cnts, key=cv2.contourArea)
        area = cv2.contourArea(largest)
        area_out = float(area)

        if area >= PINK_MIN_AREA:
            x, y, ww, hh = cv2.boundingRect(largest)
            if hh >= 12:
                cx, cy = contour_centroid_in_band(
                    largest, roi.shape[:2], PINK_BAND_FRAC, PINK_MIN_BAND_PTS
                )

                cx_full = cx
                cy_full = y0 + cy
                contour_full = largest + np.array([0, y0])

                desired_x = w - PINK_TARGET_OFFSET_RIGHT
                error = desired_x - cx_full
                steering = float(np.clip(error * PINK_STEERING_GAIN, -STEER_CLIP, STEER_CLIP))

                found = True
                status = f"PINK FOUND | err={error:+.1f} steer={steering:+.3f} area={area:.0f}"

                if not HEADLESS:
                    cv2.rectangle(display, (0, y0), (w - 1, h - 1), (255, 0, 255), 2)
                    cv2.drawContours(display, [contour_full], -1, (255, 255, 0), 2)
                    cv2.circle(display, (cx_full, cy_full), 8, (255, 255, 0), -1)
                    cv2.circle(display, (desired_x, y0 + (h - y0)//2), 8, (0, 0, 255), -1)

    return steering, display, mask, status, found, area_out

# ==========================================================
# Yellow detection helpers used by exit logic
# ==========================================================
def get_front_yellow_area(image):
    _steer, _disp, _mask, _status, found, area = yellow_follow_step(image)
    return area if found else 0.0

def get_bottom_strip_yellow_area(image):
    h, w, _ = image.shape
    strip_h = max(1, int(h * RUNOUT_BOTTOM_FRAC))
    roi = image[h - strip_h:h, 0:w]
    mask = make_yellow_mask(roi)
    return float(cv2.countNonZero(mask)), mask

# ==========================================================
# White-line perception (right camera)
# ==========================================================
def get_right_line_offset(image):
    h, w, _ = image.shape

    crop_x = int(w * 0.2)
    crop_y = h // 2
    crop = image[crop_y:h, crop_x:w]

    gray = cv2.cvtColor(crop, cv2.COLOR_BGR2GRAY)
    _, thresh = cv2.threshold(gray, WHITE_THRESH, 255, cv2.THRESH_BINARY)

    contours, _ = cv2.findContours(thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    if not contours:
        return None, thresh

    largest = max(contours, key=cv2.contourArea)
    if cv2.contourArea(largest) < 50:
        return None, thresh

    M = cv2.moments(largest)
    if M["m00"] <= 0:
        return None, thresh

    cx = int(M["m10"] / M["m00"])
    cy = int(M["m01"] / M["m00"])

    contour_full = largest + np.array([crop_x, crop_y])
    centroid_full = (crop_x + cx, crop_y + cy)

    info = {
        "contour": contour_full,
        "centroid": centroid_full,
        "crop_x": crop_x,
        "crop_y": crop_y,
        "crop_w": w - crop_x,
        "crop_h": h - crop_y,
    }
    return info, thresh

def white_follow_step(img):
    info, thresh = get_right_line_offset(img)
    h, w, _ = img.shape
    display = img.copy()

    steering = 0.0
    speed_cmd = WHITE_SPEED_MIN
    found = False
    status = "NO WHITE DETECTED"

    crop_x = int(w * 0.2)
    crop_y = h // 2
    crop_w = w - crop_x
    crop_h = h - crop_y

    if not HEADLESS:
        cv2.rectangle(display,
                      (crop_x, crop_y),
                      (crop_x + crop_w - 1, crop_y + crop_h - 1),
                      (0, 255, 255), 2)
        cv2.line(display, (crop_x, crop_y), (crop_x, crop_y + crop_h - 1), (0, 0, 255), 2)
        cv2.line(display, (crop_x + crop_w - 1, crop_y),
                 (crop_x + crop_w - 1, crop_y + crop_h - 1), (0, 255, 0), 2)

    if info is not None:
        found = True

        centroid_x, centroid_y = info["centroid"]

        target_x = int(w * 0.5) + WHITE_TARGET_OFFSET
        target_y = h // 2 + (h // 4) + 15

        dy = int(centroid_y) - int(target_y)
        steering = float(np.clip(dy * WHITE_STEERING_GAIN, -WHITE_STEER_CLIP, WHITE_STEER_CLIP))

        prop = abs(target_y - int(centroid_y)) + 1
        speed_cmd = float(np.clip(WHITE_SPEED_MAX - (WHITE_SPEED_KP * float(prop)),
                                  WHITE_SPEED_MIN, WHITE_SPEED_MAX))

        status = f"FOUND | dy={dy:+d} steer={steering:+.3f} speed={speed_cmd:.3f}"

        if not HEADLESS:
            cv2.drawContours(display, [info["contour"]], -1, (255, 0, 0), 2)
            cv2.circle(display, info["centroid"], 10, (255, 0, 0), -1)
            cv2.circle(display, (target_x, target_y), 10, (0, 0, 255), -1)

            cv2.putText(display, f"dy: {dy:+d}",
                        (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 200, 255), 2)
            cv2.putText(display, f"steer: {steering:+.3f}",
                        (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
            cv2.putText(display, f"speed: {speed_cmd:.3f}",
                        (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (200, 200, 200), 2)

    return steering, speed_cmd, display, thresh, status, found

# ==========================================================
# FSM helpers
# ==========================================================
def enter_next_motion_state(mode, odom, qcar):
    odom.reset_segment(qcar)

    if mode == MODE_Y:
        return State.YELLOW_FOLLOW

    if mode == MODE_P:
        return State.PINK_THEN_YELLOW

    if mode in (MODE_S, MODE_R):
        return State.WHITE_FOLLOW

    if mode == MODE_RY:
        return State.RIGHT_THEN_YELLOW

    if mode == MODE_L:
        return State.TURN_LEFT

    raise ValueError(f"Unknown mode '{mode}'")

def advance_after_segment(step, route_nodes, mission_stops, next_stop_idx):
    step += 1
    if step >= len(route_nodes) - 1:
        return step, State.IDLE, None, next_stop_idx

    if next_stop_idx < len(mission_stops) and route_nodes[step] == mission_stops[next_stop_idx]:
        next_stop_idx += 1
        return step, State.STOP_WAIT, time.time() + DWELL_SECONDS, next_stop_idx

    return step, State.ROUTE, None, next_stop_idx

def armed_for_visual_exit(seg_dist_m, cur_dist_target):
    if cur_dist_target <= 0.0:
        return True
    return seg_dist_m >= (EXIT_ARM_FRACTION * cur_dist_target)

def past_hard_max(seg_dist_m, cur_dist_target):
    if cur_dist_target <= 0.0:
        return False
    return seg_dist_m >= (EXIT_HARD_MAX_FRACTION * cur_dist_target)

# ==========================================================
# Main
# ==========================================================
def main():
    frame_count = 0
    fps = 0
    last_time = time.time()

    myCar = QCar()
    mission_pad = init_gamepad(GAMEPAD_INDEX)

    pickup, dropoff = ask_pickup_dropoff_gamepad(mission_pad)

    grab = FrameGrabber()
    grab.start()

    rightCam = Camera2D(
        camera_id=RIGHT_CAM_ID,
        frame_width=RIGHT_FRAME_W,
        frame_height=RIGHT_FRAME_H,
        frame_rate=RIGHT_FRAME_FPS
    )

    odom = SegmentOdom()
    odom.reset_all(myCar)

    graph = build_graph(RAW_EDGES)

    route_nodes, route_modes, route_dists, route_exits, route_left_steers = plan_full_route(graph, pickup, dropoff)
    mission_stops = [pickup, dropoff]
    next_stop_idx = 0
    need_new_mission = False

    state = State.ROUTE
    step = 0
    wait_until = None
    lost_since = None
    turn_started_t = None
    manual_advance_requested = False

    pink_mode_state = "PINK"
    right_to_yellow_seen = False
    right_to_yellow_bottom_seen = False

    find_yellow_count = 0
    yellow_end_count = 0
    yellow_runout_count = 0

    stop_hold_until = None
    stop_cooldown_until = 0.0
    obstacle_consec = 0

    if not HEADLESS:
        cv2.namedWindow(WINDOW, cv2.WINDOW_NORMAL)
        cv2.namedWindow(WIN_MASK, cv2.WINDOW_NORMAL)
        cv2.namedWindow(WIN_RIGHT, cv2.WINDOW_NORMAL)

        cv2.resizeWindow(WINDOW, 1280, 720)
        cv2.resizeWindow(WIN_MASK, 800, 240)
        cv2.resizeWindow(WIN_RIGHT, 1280, 960)

        cv2.moveWindow(WINDOW, 20, 20)
        cv2.moveWindow(WIN_MASK, 20, 780)
        cv2.moveWindow(WIN_RIGHT, 1320, 20)

    print("\nKeys: ESC/Q quit | R request new pickup/dropoff | S stop(IDLE) | SPACE advance")
    print("Mission:", " -> ".join(route_nodes))
    print("Modes:", route_modes)
    print("Dists:", route_dists)
    print("Exits:", route_exits)
    print("Left steers:", route_left_steers)
    print("Mission stops:", mission_stops)
    print(f"Pickup: {pickup}")
    print(f"Dropoff: {dropoff}")

    try:
        escalation = 0

        while True:
            cycle_start = time.time()
            now = time.time()

            _d_step, seg_dist_m, total_dist_m = odom.update(myCar)

            img = grab.get_frame()
            depth_img = grab.get_depth()

            right_img = None
            right_overlay = np.zeros((RIGHT_FRAME_H, RIGHT_FRAME_W, 3), dtype=np.uint8)
            try:
                rightCam.read()
                if rightCam.image_data is not None and rightCam.image_data.size != 0:
                    right_img = rightCam.image_data.copy()
                    right_overlay = right_img.copy()
            except Exception:
                right_img = None

            if img is None:
                neutral_brake(myCar)
                if (time.time() - grab.last_good_t) > 2.0:
                    escalation += 1

                if escalation >= MAX_ESCALATION:
                    newCar = reopen_qcar(myCar)
                    if newCar is None:
                        os.execv(sys.executable, [sys.executable] + sys.argv)
                    else:
                        myCar = newCar
                        odom.reset_all(myCar)
                    escalation = 0

                time.sleep(0.02)
                continue
            else:
                escalation = 0

            overlay = img.copy()
            mask_show = np.zeros((100, 300), dtype=np.uint8)
            forward_cmd = 0.0
            steering_cmd = 0.0
            braking = False

            frame_count += 1
            if now - last_time >= 1.0:
                fps = frame_count
                frame_count = 0
                last_time = now

            cur_src = route_nodes[step] if step < len(route_nodes) else "?"
            cur_dst = route_nodes[step + 1] if step + 1 < len(route_nodes) else "?"
            cur_mode = route_modes[step] if step < len(route_modes) else "-"
            cur_dist_target = route_dists[step] if step < len(route_dists) else 0.0
            cur_exit_mode = route_exits[step] if step < len(route_exits) else "-"
            cur_left_steer = route_left_steers[step] if step < len(route_left_steers) else None

            manual_advance = manual_advance_requested
            manual_advance_requested = False

            if state == State.ROUTE:
                find_yellow_count = 0
                yellow_end_count = 0
                yellow_runout_count = 0
                pink_mode_state = "PINK"
                right_to_yellow_seen = False
                right_to_yellow_bottom_seen = False
                obstacle_consec = 0

            if state == State.IDLE:
                braking = True
                cv2.putText(overlay, "STATE: IDLE (Press R for new mission)", (10, 30),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)

                if need_new_mission:
                    pickup, dropoff = ask_pickup_dropoff_gamepad(mission_pad)

                    route_nodes, route_modes, route_dists, route_exits, route_left_steers = plan_full_route(graph, pickup, dropoff)
                    mission_stops = [pickup, dropoff]
                    next_stop_idx = 0
                    step = 0
                    wait_until = None
                    lost_since = None
                    turn_started_t = None
                    find_yellow_count = 0
                    yellow_end_count = 0
                    yellow_runout_count = 0
                    pink_mode_state = "PINK"
                    right_to_yellow_seen = False
                    right_to_yellow_bottom_seen = False
                    stop_hold_until = None
                    stop_cooldown_until = 0.0
                    obstacle_consec = 0
                    odom.reset_all(myCar)
                    need_new_mission = False
                    state = State.ROUTE

                    print("Mission:", " -> ".join(route_nodes))
                    print("Modes:", route_modes)
                    print("Dists:", route_dists)
                    print("Exits:", route_exits)
                    print("Left steers:", route_left_steers)
                    print("Mission stops:", mission_stops)
                    print(f"Pickup: {pickup}")
                    print(f"Dropoff: {dropoff}")

            elif state == State.ROUTE:
                if step >= len(route_nodes) - 1:
                    state = State.IDLE
                else:
                    cur_src = route_nodes[step]
                    cur_dst = route_nodes[step + 1]
                    cur_mode = route_modes[step]
                    cur_dist_target = route_dists[step]
                    cur_exit_mode = route_exits[step]
                    cur_left_steer = route_left_steers[step]

                    state = enter_next_motion_state(cur_mode, odom, myCar)
                    lost_since = None
                    turn_started_t = None
                    pink_mode_state = "PINK"
                    right_to_yellow_seen = False
                    right_to_yellow_bottom_seen = False

                    cv2.putText(overlay, f"STATE: ROUTE | Dispatch {cur_mode} {cur_src}->{cur_dst} | exit={cur_exit_mode}",
                                (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)

            elif state == State.YELLOW_FOLLOW:
                t0 = time.time()
                steering_cmd, overlay, mask_show, status, found, _yellow_area = yellow_follow_step(img)
                calc_ms = (time.time() - t0) * 1000.0

                stop_seen = False
                stop_dist = math.nan

                if STOP_ENABLE and depth_img is not None:
                    stop_seen, stop_overlay, stop_mask, stop_dist = detect_stop_sign(img, depth_img)
                    if stop_seen:
                        overlay = stop_overlay

                if stop_hold_until is not None and now < stop_hold_until:
                    forward_cmd = 0.0
                    steering_cmd = 0.0
                    braking = True
                    status = f"STOP_WAIT_SIGN {max(0.0, stop_hold_until - now):.1f}s"
                else:
                    if stop_hold_until is not None and now >= stop_hold_until:
                        stop_hold_until = None

                    can_trigger_stop = (now >= stop_cooldown_until)

                    if stop_seen and can_trigger_stop:
                        stop_hold_until = now + STOP_DURATION_S
                        stop_cooldown_until = now + STOP_COOLDOWN_S
                        forward_cmd = 0.0
                        steering_cmd = 0.0
                        braking = True
                        status = f"STOP_SIGN {stop_dist:.2f}m"
                    else:
                        if found:
                            lost_since = None
                            forward_cmd = SPEED_YELLOW
                        else:
                            if lost_since is None:
                                lost_since = now
                            if (now - lost_since) >= LOST_TIMEOUT_S:
                                forward_cmd = LOST_SPEED_CMD
                                steering_cmd = 0.0
                            else:
                                forward_cmd = SPEED_YELLOW * 0.5
                                steering_cmd = 0.0

                arrived = False
                reason = ""

                if cur_exit_mode == EXIT_ENCODER:
                    if cur_dist_target > 0.0:
                        arrived = seg_dist_m >= cur_dist_target
                        reason = "encoder"
                elif cur_exit_mode == EXIT_YELLOW_END:
                    if armed_for_visual_exit(seg_dist_m, cur_dist_target):
                        if not found:
                            yellow_end_count += 1
                        else:
                            yellow_end_count = 0

                        if yellow_end_count >= YELLOW_END_CONFIRM_FRAMES:
                            arrived = True
                            reason = "yellow_end"

                    if past_hard_max(seg_dist_m, cur_dist_target):
                        arrived = True
                        reason = "hard_max"

                if manual_advance:
                    arrived = True
                    reason = "manual"

                cv2.putText(overlay, f"STATE: YELLOW_FOLLOW | {cur_src}->{cur_dst} | {status}",
                            (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
                cv2.putText(overlay, f"exit={cur_exit_mode} seg: {seg_dist_m:.3f}/{cur_dist_target:.3f} m | total: {total_dist_m:.3f} m",
                            (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
                cv2.putText(overlay, f"FPS:{fps} Calc:{calc_ms:.1f}ms steer:{steering_cmd:+.3f}",
                            (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                cv2.putText(overlay, f"yellow_end_count={yellow_end_count}/{YELLOW_END_CONFIRM_FRAMES} reason={reason}",
                            (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 165, 255), 2)

                if stop_hold_until is not None:
                    cv2.putText(overlay, f"stop_hold={max(0.0, stop_hold_until - now):.1f}s cooldown={max(0.0, stop_cooldown_until - now):.1f}s",
                                (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 165, 255), 2)

                if arrived and stop_hold_until is None:
                    step, state, wait_until, next_stop_idx = advance_after_segment(step, route_nodes, mission_stops, next_stop_idx)

            elif state == State.PINK_THEN_YELLOW:
                t0 = time.time()

                y_steer, y_overlay, y_mask, y_status, y_found, y_area = yellow_follow_step(img)
                p_steer, p_overlay, p_mask, p_status, p_found, p_area = pink_follow_step(img)

                calc_ms = (time.time() - t0) * 1000.0

                if y_found:
                    overlay = y_overlay
                    mask_show = y_mask
                    status = y_status
                    forward_cmd = SPEED_YELLOW
                    steering_cmd = y_steer
                    pink_mode_state = "YELLOW"
                    lost_since = None

                elif p_found:
                    overlay = p_overlay
                    mask_show = p_mask
                    status = p_status
                    forward_cmd = SPEED_PINK
                    steering_cmd = p_steer
                    pink_mode_state = "PINK"
                    lost_since = None

                else:
                    overlay = p_overlay
                    mask_show = p_mask
                    status = "NO YELLOW / NO PINK"
                    forward_cmd = SPEED_SEARCH
                    steering_cmd = 0.0

                    if lost_since is None:
                        lost_since = now

                arrived = False
                reason = ""

                front_yellow_area = get_front_yellow_area(img)
                front_yellow_found = front_yellow_area >= FIND_YELLOW_MIN_AREA

                if cur_exit_mode == EXIT_FIND_YELLOW:
                    if armed_for_visual_exit(seg_dist_m, cur_dist_target):
                        if front_yellow_found:
                            find_yellow_count += 1
                        else:
                            find_yellow_count = 0

                        if find_yellow_count >= FIND_YELLOW_CONFIRM_FRAMES:
                            arrived = True
                            reason = "find_yellow"

                    if past_hard_max(seg_dist_m, cur_dist_target):
                        arrived = True
                        reason = "hard_max"

                elif cur_exit_mode == EXIT_ENCODER:
                    if cur_dist_target > 0.0:
                        arrived = seg_dist_m >= cur_dist_target
                        reason = "encoder"

                elif cur_exit_mode == EXIT_YELLOW_END:
                    if armed_for_visual_exit(seg_dist_m, cur_dist_target):
                        if not y_found:
                            yellow_end_count += 1
                        else:
                            yellow_end_count = 0

                        if yellow_end_count >= YELLOW_END_CONFIRM_FRAMES:
                            arrived = True
                            reason = "yellow_end"

                    if past_hard_max(seg_dist_m, cur_dist_target):
                        arrived = True
                        reason = "hard_max"

                if manual_advance:
                    arrived = True
                    reason = "manual"

                cv2.putText(overlay, f"STATE: PINK_THEN_YELLOW | {cur_src}->{cur_dst} | phase={pink_mode_state}",
                            (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
                cv2.putText(overlay, f"{status}",
                            (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
                cv2.putText(overlay, f"exit={cur_exit_mode} seg: {seg_dist_m:.3f}/{cur_dist_target:.3f} m | total: {total_dist_m:.3f} m",
                            (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
                cv2.putText(overlay, f"FPS:{fps} Calc:{calc_ms:.1f}ms steer:{steering_cmd:+.3f} speed:{forward_cmd:.3f}",
                            (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                cv2.putText(overlay, f"front_yellow_area={front_yellow_area:.0f} count={find_yellow_count}/{FIND_YELLOW_CONFIRM_FRAMES} reason={reason}",
                            (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 165, 255), 2)

                if arrived:
                    step, state, wait_until, next_stop_idx = advance_after_segment(step, route_nodes, mission_stops, next_stop_idx)

            elif state == State.WHITE_FOLLOW:
                t0 = time.time()

                if right_img is None:
                    steering_cmd = 0.0
                    forward_cmd = 0.0
                    white_found = False
                    status = "RIGHT CAMERA MISSING"
                    calc_ms = (time.time() - t0) * 1000.0
                    braking = True
                else:
                    steering_cmd, forward_cmd, right_overlay, _right_thresh, status, white_found = white_follow_step(right_img)
                    calc_ms = (time.time() - t0) * 1000.0

                    if not white_found:
                        forward_cmd = WHITE_SPEED_MIN
                        steering_cmd = 0.0

                arrived = False
                reason = ""

                front_yellow_area = get_front_yellow_area(img)
                front_yellow_found = front_yellow_area >= FIND_YELLOW_MIN_AREA

                bottom_yellow_area, _bottom_mask = get_bottom_strip_yellow_area(img)
                bottom_yellow_present = bottom_yellow_area >= RUNOUT_MIN_AREA

                if cur_exit_mode == EXIT_FIND_YELLOW:
                    if armed_for_visual_exit(seg_dist_m, cur_dist_target):
                        if front_yellow_found:
                            find_yellow_count += 1
                        else:
                            find_yellow_count = 0

                        if find_yellow_count >= FIND_YELLOW_CONFIRM_FRAMES:
                            arrived = True
                            reason = "find_yellow"

                    if past_hard_max(seg_dist_m, cur_dist_target):
                        arrived = True
                        reason = "hard_max"

                elif cur_exit_mode == EXIT_YELLOW_RUNOUT_BOTTOM:
                    if armed_for_visual_exit(seg_dist_m, cur_dist_target):
                        if not bottom_yellow_present:
                            yellow_runout_count += 1
                        else:
                            yellow_runout_count = 0

                        if yellow_runout_count >= RUNOUT_CONFIRM_FRAMES:
                            arrived = True
                            reason = "yellow_runout_bottom"

                    if past_hard_max(seg_dist_m, cur_dist_target):
                        arrived = True
                        reason = "hard_max"

                elif cur_exit_mode == EXIT_ENCODER:
                    if cur_dist_target > 0.0:
                        arrived = seg_dist_m >= cur_dist_target
                        reason = "encoder"

                if manual_advance:
                    arrived = True
                    reason = "manual"

                cv2.putText(overlay, f"STATE: WHITE_FOLLOW | {cur_src}->{cur_dst} | {status}",
                            (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
                cv2.putText(overlay, f"exit={cur_exit_mode} seg: {seg_dist_m:.3f}/{cur_dist_target:.3f} m | total: {total_dist_m:.3f} m",
                            (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
                cv2.putText(overlay, f"FPS:{fps} Calc:{calc_ms:.1f}ms steer:{steering_cmd:+.3f} speed:{forward_cmd:.3f}",
                            (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                cv2.putText(overlay, f"front_yellow_area={front_yellow_area:.0f} find_count={find_yellow_count}/{FIND_YELLOW_CONFIRM_FRAMES}",
                            (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 165, 255), 2)
                cv2.putText(overlay, f"bottom_yellow={bottom_yellow_area:.0f} runout_count={yellow_runout_count}/{RUNOUT_CONFIRM_FRAMES} reason={reason}",
                            (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 165, 255), 2)

                if arrived:
                    step, state, wait_until, next_stop_idx = advance_after_segment(step, route_nodes, mission_stops, next_stop_idx)

            elif state == State.RIGHT_THEN_YELLOW:
                t0 = time.time()

                if right_img is None:
                    w_steer = 0.0
                    w_speed = 0.0
                    white_found = False
                    w_status = "RIGHT CAMERA MISSING"
                    right_overlay_local = right_overlay
                    braking = True
                else:
                    w_steer, w_speed, right_overlay_local, _right_thresh, w_status, white_found = white_follow_step(right_img)
                    if not white_found:
                        w_speed = WHITE_SPEED_MIN
                        w_steer = 0.0

                y_steer, y_overlay, y_mask, y_status, y_found, y_area = yellow_follow_step(img)

                calc_ms = (time.time() - t0) * 1000.0
                arm_dist = RIGHT_TO_YELLOW_ARM_FRAC * cur_dist_target if cur_dist_target > 0.0 else 0.0
                armed = (cur_dist_target <= 0.0) or (seg_dist_m >= arm_dist)

                bottom_yellow_area, _bottom_mask = get_bottom_strip_yellow_area(img)
                bottom_yellow_present = bottom_yellow_area >= RUNOUT_MIN_AREA

                if armed and y_found:
                    right_to_yellow_seen = True

                if right_to_yellow_seen and bottom_yellow_present:
                    right_to_yellow_bottom_seen = True

                if right_to_yellow_seen:
                    overlay = y_overlay
                    mask_show = y_mask

                    if y_found:
                        steering_cmd = y_steer
                        forward_cmd = SPEED_YELLOW
                        status = f"YELLOW ACTIVE | {y_status}"
                        lost_since = None
                    else:
                        status = "YELLOW LOST AFTER HANDOFF"
                        if lost_since is None:
                            lost_since = now
                        if (now - lost_since) >= LOST_TIMEOUT_S:
                            forward_cmd = LOST_SPEED_CMD
                            steering_cmd = 0.0
                        else:
                            forward_cmd = SPEED_YELLOW * 0.5
                            steering_cmd = 0.0
                else:
                    overlay = img.copy()
                    mask_show = np.zeros((100, 300), dtype=np.uint8)

                    if right_img is not None:
                        rh, rw = right_overlay_local.shape[:2]
                        overlay[0:rh, 0:rw] = right_overlay_local

                    steering_cmd = w_steer
                    forward_cmd = w_speed
                    status = f"RIGHT ACTIVE | {w_status}"

                    if armed:
                        status += " | ARMED FOR YELLOW"

                arrived = False
                reason = ""

                if cur_exit_mode == EXIT_YELLOW_RUNOUT_BOTTOM:
                    if right_to_yellow_bottom_seen:
                        if not bottom_yellow_present:
                            yellow_runout_count += 1
                        else:
                            yellow_runout_count = 0

                        if yellow_runout_count >= RUNOUT_CONFIRM_FRAMES:
                            arrived = True
                            reason = "yellow_runout_bottom"

                    if past_hard_max(seg_dist_m, cur_dist_target):
                        arrived = True
                        reason = "hard_max"

                elif cur_exit_mode == EXIT_YELLOW_END:
                    if right_to_yellow_seen:
                        if not y_found:
                            yellow_end_count += 1
                        else:
                            yellow_end_count = 0

                        if yellow_end_count >= YELLOW_END_CONFIRM_FRAMES:
                            arrived = True
                            reason = "yellow_end"

                    if past_hard_max(seg_dist_m, cur_dist_target):
                        arrived = True
                        reason = "hard_max"

                elif cur_exit_mode == EXIT_ENCODER:
                    if cur_dist_target > 0.0:
                        arrived = seg_dist_m >= cur_dist_target
                        reason = "encoder"

                if manual_advance:
                    arrived = True
                    reason = "manual"

                cv2.putText(overlay, f"STATE: RIGHT_THEN_YELLOW | {cur_src}->{cur_dst} | armed={armed} | seen_yellow={right_to_yellow_seen} | bottom_seen={right_to_yellow_bottom_seen}",
                            (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
                cv2.putText(overlay, f"{status}",
                            (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
                cv2.putText(overlay, f"seg: {seg_dist_m:.3f}/{cur_dist_target:.3f} m | arm@ {arm_dist:.3f} m | total: {total_dist_m:.3f} m",
                            (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
                cv2.putText(overlay, f"FPS:{fps} Calc:{calc_ms:.1f}ms steer:{steering_cmd:+.3f} speed:{forward_cmd:.3f}",
                            (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                cv2.putText(overlay, f"bottom_yellow={bottom_yellow_area:.0f} bottom_seen={right_to_yellow_bottom_seen} runout_count={yellow_runout_count}/{RUNOUT_CONFIRM_FRAMES} reason={reason}",
                            (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 165, 255), 2)

                if arrived:
                    step, state, wait_until, next_stop_idx = advance_after_segment(step, route_nodes, mission_stops, next_stop_idx)

            elif state == State.TURN_LEFT:
                if turn_started_t is None:
                    turn_started_t = now

                dt = now - turn_started_t
                forward_cmd = SPEED_TURN
                steering_cmd = cur_left_steer if cur_left_steer is not None else TURN_LEFT_STEER

                arrived = False
                reason = ""

                front_yellow_area = get_front_yellow_area(img)
                front_yellow_found = front_yellow_area >= FIND_YELLOW_MIN_AREA

                bottom_yellow_area, _bottom_mask = get_bottom_strip_yellow_area(img)
                bottom_yellow_present = bottom_yellow_area >= RUNOUT_MIN_AREA

                if cur_exit_mode == EXIT_FIND_YELLOW:
                    if armed_for_visual_exit(seg_dist_m, cur_dist_target):
                        if front_yellow_found:
                            find_yellow_count += 1
                        else:
                            find_yellow_count = 0

                        if find_yellow_count >= FIND_YELLOW_CONFIRM_FRAMES:
                            arrived = True
                            reason = "find_yellow"

                    if past_hard_max(seg_dist_m, cur_dist_target):
                        arrived = True
                        reason = "hard_max"

                elif cur_exit_mode == EXIT_YELLOW_RUNOUT_BOTTOM:
                    if armed_for_visual_exit(seg_dist_m, cur_dist_target):
                        if not bottom_yellow_present:
                            yellow_runout_count += 1
                        else:
                            yellow_runout_count = 0

                        if yellow_runout_count >= RUNOUT_CONFIRM_FRAMES:
                            arrived = True
                            reason = "yellow_runout_bottom"

                    if past_hard_max(seg_dist_m, cur_dist_target):
                        arrived = True
                        reason = "hard_max"

                elif cur_exit_mode == EXIT_ENCODER:
                    if cur_dist_target > 0.0:
                        arrived = seg_dist_m >= cur_dist_target
                        reason = "encoder"

                else:
                    if cur_dist_target > 0.0:
                        arrived = seg_dist_m >= cur_dist_target
                        reason = "encoder_default"
                    else:
                        arrived = dt >= TURN_LEFT_TIME_S
                        reason = "timer_fallback"

                if manual_advance:
                    arrived = True
                    reason = "manual"

                cv2.putText(overlay, f"STATE: TURN_LEFT | {cur_src}->{cur_dst}",
                            (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
                cv2.putText(overlay, f"steer={steering_cmd:+.3f} exit={cur_exit_mode} seg:{seg_dist_m:.3f}/{cur_dist_target:.3f}m dt:{dt:.2f}s",
                            (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
                cv2.putText(overlay, f"front_yellow={front_yellow_area:.0f} find_count={find_yellow_count}/{FIND_YELLOW_CONFIRM_FRAMES}",
                            (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                cv2.putText(overlay, f"bottom_yellow={bottom_yellow_area:.0f} runout_count={yellow_runout_count}/{RUNOUT_CONFIRM_FRAMES} reason={reason}",
                            (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 165, 255), 2)

                if arrived:
                    step, state, wait_until, next_stop_idx = advance_after_segment(step, route_nodes, mission_stops, next_stop_idx)
                    turn_started_t = None

            elif state == State.TURN_RIGHT:
                braking = True
                cv2.putText(overlay, "STATE: TURN_RIGHT (unused)", (10, 30),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)

            elif state == State.STOP_WAIT:
                braking = True
                remaining = max(0.0, (wait_until - now) if wait_until is not None else 0.0)

                cv2.putText(overlay, f"STATE: STOP_WAIT at {route_nodes[step]} | {remaining:.1f}s left",
                            (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)

                if wait_until is not None and now >= wait_until:
                    wait_until = None
                    state = State.ROUTE

            mission_str = " -> ".join(route_nodes) if route_nodes else "(no route)"
            cv2.putText(overlay, f"Mission: {mission_str}",
                        (10, overlay.shape[0] - 20),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 255, 255), 1)

            # ==========================================================
            # Global obstacle override
            # Keep steering, force speed = 0 when obstacle is confirmed
            # ==========================================================
            moving_states = {
                State.YELLOW_FOLLOW,
                State.PINK_THEN_YELLOW,
                State.WHITE_FOLLOW,
                State.RIGHT_THEN_YELLOW,
                State.TURN_LEFT,
            }

            obstacle_active = False
            obstacle_dist = math.nan

            if state in moving_states and img is not None:
                hF, wF = img.shape[:2]
                obstacle_now = detect_obstacle(img, depth_img, hF, wF)
                obstacle_dist = obstacle_p10(depth_img, hF, wF)

                if obstacle_now:
                    obstacle_consec = min(OBSTACLE_CONSEC_N, obstacle_consec + 1)
                else:
                    obstacle_consec = 0

                obstacle_active = (obstacle_consec >= OBSTACLE_CONSEC_N)

                draw_obstacle_roi(overlay, hF, wF, obstacle_active or obstacle_now, obstacle_dist)

                if obstacle_active and not braking:
                    forward_cmd = 0.0
                    cv2.putText(overlay, "OBSTACLE HOLD: speed=0, steering kept",
                                (10, 180), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
            else:
                obstacle_consec = 0

            cycle_dt = time.time() - cycle_start
            try:
                if cycle_dt > MAX_LOOP_TIME_S:
                    neutral_brake(myCar)
                else:
                    drive(myCar,
                          speed_cmd=forward_cmd if not braking else 0.0,
                          steering_cmd=np.clip(steering_cmd, -STEER_CLIP, STEER_CLIP),
                          brake=braking)
            except Exception:
                neutral_brake(myCar)
                newCar = reopen_qcar(myCar)
                if newCar is None:
                    escalation += 1
                    if escalation >= MAX_ESCALATION:
                        os.execv(sys.executable, [sys.executable] + sys.argv)
                else:
                    myCar = newCar
                    odom.reset_all(myCar)
                    escalation = 0

            if not HEADLESS:
                cv2.imshow(WINDOW, overlay)
                cv2.imshow(WIN_MASK, mask_show)
                cv2.imshow(WIN_RIGHT, right_overlay)

                k = cv2.waitKey(1) & 0xFF
                if k in (27, ord('q'), ord('Q')):
                    print("Quit requested.")
                    break
                elif k in (ord('s'), ord('S')):
                    state = State.IDLE
                    route_nodes = []
                    route_modes = []
                    route_dists = []
                    route_exits = []
                    route_left_steers = []
                    mission_stops = []
                    next_stop_idx = 0
                    step = 0
                    wait_until = None
                    lost_since = None
                    turn_started_t = None
                    find_yellow_count = 0
                    yellow_end_count = 0
                    yellow_runout_count = 0
                    pink_mode_state = "PINK"
                    right_to_yellow_seen = False
                    right_to_yellow_bottom_seen = False
                    stop_hold_until = None
                    stop_cooldown_until = 0.0
                    obstacle_consec = 0
                    next_stop_idx = 0
                    need_new_mission = False
                    neutral_brake(myCar)
                    print("Stopped. (IDLE)")
                elif k in (ord('r'), ord('R')):
                    state = State.IDLE
                    need_new_mission = True
                    step = 0
                    wait_until = None
                    lost_since = None
                    turn_started_t = None
                    find_yellow_count = 0
                    yellow_end_count = 0
                    yellow_runout_count = 0
                    pink_mode_state = "PINK"
                    right_to_yellow_seen = False
                    right_to_yellow_bottom_seen = False
                    stop_hold_until = None
                    stop_cooldown_until = 0.0
                    obstacle_consec = 0
                    next_stop_idx = 0
                    neutral_brake(myCar)
                    print("Requesting new pickup/dropoff from gamepad...")
                elif k == 32:
                    manual_advance_requested = True
                    print("[Advance] manual segment advance requested")
            else:
                time.sleep(0.01)

    finally:
        try:
            cv2.destroyAllWindows()
        except Exception:
            pass
        try:
            grab.stop()
        except Exception:
            pass
        try:
            safe_terminate_camera2d(rightCam)
        except Exception:
            pass
        try:
            if mission_pad is not None:
                mission_pad.terminate()
        except Exception:
            pass
        try:
            neutral_brake(myCar)
        except Exception:
            pass
        try:
            myCar.terminate()
        except Exception:
            pass

if __name__ == "__main__":
    main()