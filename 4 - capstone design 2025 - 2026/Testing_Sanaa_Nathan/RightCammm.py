...
speed = 0
steering_gain = 0  # Gain used for steering calculation (increased per user request)
max_steering_angle = 0  # Maximum steering angle in degrees (mechanical limit)
...
try:
    while True:  # Main control loop
        ...
        # Recreate cropped region used by processing so we can show it
        cropped = img[crop_y:bottom_crop, crop_x:right_crop].copy()
        panel_cropped = cropped

        # Cropped grayscale (single channel) and a BGR copy for tiled display
        try:
            panel_cropped_gray = cv2.cvtColor(panel_cropped, cv2.COLOR_BGR2GRAY)
            panel_cropped_gray_bgr = cv2.cvtColor(panel_cropped_gray, cv2.COLOR_GRAY2BGR)
        except Exception:
            panel_cropped_gray = np.zeros((panel_cropped.shape[0], panel_cropped.shape[1]), dtype=np.uint8)
            panel_cropped_gray_bgr = np.zeros_like(panel_cropped)

        # Threshold (use the thresh returned from the processing function)
        try:
            panel_thresh = thresh  # already single-channel and matches cropped size
            panel_thresh_bgr = cv2.cvtColor(panel_thresh, cv2.COLOR_GRAY2BGR)
        except Exception:
            panel_thresh = np.zeros((panel_cropped.shape[0], panel_cropped.shape[1]), dtype=np.uint8)
            panel_thresh_bgr = np.zeros_like(panel_cropped)

        # Contours overlaid on cropped color image
        try:
            cnts, _ = cv2.findContours(panel_thresh.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            panel_contours = panel_cropped.copy()
            if cnts:
                cv2.drawContours(panel_contours, cnts, -1, (0, 0, 255), 2)
        except Exception:
            panel_contours = np.zeros_like(panel_cropped)

        # Resize panels to a common size for tiled display
        target_w = 420
        target_h = 320
        def fit_panel(p):
            try:
        ...
        cv2.namedWindow('Left Camera MultiView', cv2.WINDOW_NORMAL)
        cv2.resizeWindow('Left Camera MultiView', 1280, 960)
        cv2.imshow('Left Camera MultiView', tiled)
