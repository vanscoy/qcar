...
# ------------------ Initial open-loop turn params & helper ------------------
# Configure these values to experiment with an initial left turn before
# handing control off to the closed-loop line follower.
# - INIT_TURN_TIME: seconds to run the open-loop turn
...
def do_initial_turn(qcar, duration=INIT_TURN_TIME, speed_m=INIT_TURN_SPEED, steering_cmd=INIT_TURN_STEERING,
    ...
    """Run an open-loop turn for `duration` seconds by repeatedly sending
    the [speed, steering] motor command. This is intended for experimental
    tuning; the function is defensive against SDK errors.
    """
    if leds is None:
        leds = np.array([0, 0, 0, 0, 0, 0, 1, 1])
    cmd = np.array([float(speed_m), float(steering_cmd)])
    end_t = time.time() + float(duration)
    period = 1.0 / float(update_hz)
    try:
        while time.time() < end_t:
            try:
                qcar.read_write_std(cmd, leds)
            except Exception:
                # ignore SDK errors during experimental turn
                pass
            time.sleep(period)
    finally:
        # Always attempt to stop the motors at the end of the manoeuvre
        try:
            qcar.read_write_std(np.array([0.0, 0.0]), leds)
        except Exception:
            pass

# Run an initial open-loop left turn so you can experimentally tune steering
# and timing. This executes once, then falls through to the main control loop.
try:
    do_initial_turn(myCar, duration=INIT_TURN_TIME, speed_m=INIT_TURN_SPEED, steering_cmd=INIT_TURN_STEERING)
